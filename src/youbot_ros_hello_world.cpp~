...
//These are the global variables we are getting from the object_detection.py code
int Pepsi_x_cent = 0;
int Pepsi_y_cent = 0;
...
void centroidsCallback(const youbot_ros_hello_world::Num::ConstPtr& msg)
{
	...
	//setting the global variables to the 
...
//Moves the arm to our home position |*-*- position
	jointvalues[0] = 2.95;
	jointvalues[1] = 1.12;
	jointvalues[2] = -4.144;
	jointvalues[3] = 1.764;
...
//makes sure the youbot is stopped 
    twist.linear.x = 0;
	twist.linear.y = 0;	
	twist.angular.z = 0;
...
//will be a number of pixels in the future
		...
		//moves the youbot to the left
			...
			//publish the info to move the base of the youbot
			twist.linear.x = xinc;
	   		twist.linear.y = yinc;
			platformPublisher.publish(twist);
			cin >> adjust;
...
//used to tell the youbot where to move in reference to our home position 
void forward_kin()
{
	//for the base
	geometry_msgs::Twist twist;
